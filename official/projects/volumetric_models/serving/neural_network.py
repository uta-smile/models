...
class SegmentationNetwork(NeuralNetwork):
            ...
            with torch.no_grad():
                if self.conv_op == nn.Conv3d:
                    if use_sliding_window:
                        ...
                        res = self._internal_predict_3D_3Dconv_tiled(model_fn, x, step_size, do_mirroring, mirror_axes, patch_size,
                    ...
                    if use_sliding_window:
                        ...
                        imported = tf.saved_model.load(export_dir_path)
                        model_fn = imported.signatures['serving_default']
                        res = self._internal_predict_3D_2Dconv_tiled(model_fn, x, patch_size, do_mirroring, mirror_axes, step_size,
                                                                     regions_class_order, use_gaussian, pad_border_mode,
    ...
    def _internal_predict_3D_3Dconv_tiled(self, model_fn, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
    ...
    def _internal_maybe_mirror_and_pred_3D(self, model_fn, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        # print(self.inference_apply_nonlin(self(x)), type(self.inference_apply_nonlin(self(x))), self.inference_apply_nonlin(self(x)).shape)
        ...
        # export_dir_path = '/home/feng/Desktop/models-master/official/vision/beta/projects/volumetric_models/serving/exported_model_2500/saved_model'

        # b, c, h, y, z = x.shape
        # torch_input = x.view(b, h, y, z, c)
        ...
        # b, h, y, z, c = torch_output.shape
        # torch_output = torch_output.view(b, c, h, y, z)

        # print(torch_output, type(torch_output), torch_output.shape)
        # print(self.inference_apply_nonlin(torch_output), type(self.inference_apply_nonlin(torch_output)), self.inference_apply_nonlin(torch_output).shape)

        def inf(x):
            shp_b, shp_c, shp_x, shp_y, shp_z = x.shape
            torch_input = x.view(shp_b, shp_x, shp_y, shp_z, shp_c)
            np_input = torch_input.cpu().numpy()
            tf_input = tf.convert_to_tensor(np_input)
            tf_output = model_fn(inputs=tf_input)
            np_output = tf_output['logits'].numpy()
            torch_output = torch.from_numpy(np_output)
            torch_output = torch_output.view(shp_b, shp_c, shp_x, shp_y, shp_z)
            torch_output = to_cuda(maybe_to_torch(torch_output), gpu_id=self.get_device())
            return torch_output

        for m in range(mirror_idx):
            if m == 0:
                # pred = self.inference_apply_nonlin(self(x))
                pred = self.inference_apply_nonlin(inf(x))
                result_torch += 1 / num_results * pred

            if m == 1 and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, ))))
                pred = self.inference_apply_nonlin(inf(torch.flip(x, (4, ))))
                result_torch += 1 / num_results * torch.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (3, ))))
                pred = self.inference_apply_nonlin(inf(torch.flip(x, (3, ))))
                result_torch += 1 / num_results * torch.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3))))
                pred = self.inference_apply_nonlin(inf(torch.flip(x, (4, 3))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (2, ))))
                pred = self.inference_apply_nonlin(inf(torch.flip(x, (2, ))))
                result_torch += 1 / num_results * torch.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2))))
                pred = self.inference_apply_nonlin(inf(torch.flip(x, (4, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2))))
                pred = self.inference_apply_nonlin(inf(torch.flip(x, (3, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                # pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2))))
                pred = self.inference_apply_nonlin(inf(torch.flip(x, (4, 3, 2))))
    ...
    def _internal_predict_2D_2Dconv_tiled(self, model_fn, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          patch_size: tuple, regions_class_order: tuple, use_gaussian: bool,
                                          pad_border_mode: str, pad_kwargs: dict, all_in_gpu: bool,
                                          verbose: bool) -> Tuple[np.ndarray, np.ndarray]:
        # better safe than sorry
    ...
    def _internal_predict_3D_2Dconv_tiled(self, model_fn, x: np.ndarray, patch_size: Tuple[int, int], do_mirroring: bool,
