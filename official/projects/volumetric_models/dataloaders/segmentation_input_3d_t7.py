...
class Parser(parser.Parser):
  ...
  def _data_augmentation_tr(self, image, label):
    ...
    data_dict = DataLoader3D(data, patch_size=[40, 263, 263], final_patch_size=[40, 224, 224], batch_size=1,
  ...
  def _data_augmentation_val(self, image, label):
    ...
    label = label[np.newaxis, :]
    data = np.concatenate((image, label), axis=0)
    data_dict = DataLoader3D(data, patch_size=[40, 224, 224], final_patch_size=[40, 224, 224], batch_size=1,
                         has_prev_stage=False, pad_mode="constant", pad_sides=None,
                         memmap_mode='r').generate_train_batch()
    image = data_dict['data']
    label = data_dict['seg']
...
def tr_transforms(image, label):
    ...
    data_dict = SpatialTransform(patch_size=[224, 224], patch_center_dist_from_border=None,
