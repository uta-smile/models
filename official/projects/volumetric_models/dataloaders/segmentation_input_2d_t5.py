...
class Parser(parser.Parser):
  ...
  def _data_augmentation_tr(self, image, label):
    ...
    data_dict = DataLoader2D(data, patch_size=[376, 376], final_patch_size=[320, 320], batch_size=1,
  ...
  def _data_augmentation_val(self, image, label):
    ...
    label = label[np.newaxis, :]
    data = np.concatenate((image, label), axis=0)
    data_dict = DataLoader2D(data, patch_size=[320, 320], final_patch_size=[320, 320], batch_size=1,
                             pad_mode="constant", pad_sides=None, memmap_mode='r').generate_train_batch()
    image = data_dict['data']
    label = data_dict['seg']
...
def tr_transforms(image, label):
    ...
    data_dict = SpatialTransform(patch_size=[320, 320], patch_center_dist_from_border=None,
