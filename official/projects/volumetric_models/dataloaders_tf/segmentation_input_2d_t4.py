...
from official.projects.volumetric_models.data_augmentations_tf.tfda_2d.transforms.spatial_transforms import SpatialTransform, MirrorTransform, SpatialTransform2D, MirrorTransform2D
from official.projects.volumetric_models.data_augmentations_tf.tfda_2d.transforms.noise_transforms import GaussianNoiseTransform, GaussianBlurTransform, GaussianBlurTransform2D
from official.projects.volumetric_models.data_augmentations_tf.tfda_2d.transforms.color_transforms import BrightnessMultiplicativeTransform, ContrastAugmentationTransform, GammaTransform
from official.projects.volumetric_models.data_augmentations_tf.tfda_2d.transforms.custom_transforms import MaskTransform, OneHotTransform, Convert3DTo2DTransform, Convert2DTo3DTransform, OneHotTransform2D
from official.projects.volumetric_models.data_augmentations_tf.tfda_2d.transforms.utility_transforms import RemoveLabelTransform
from official.projects.volumetric_models.data_augmentations_tf.tfda_2d.transforms.resample_transforms import SimulateLowResolutionTransform, SimulateLowResolutionTransform2D
from official.projects.volumetric_models.data_augmentations_tf.tfda_2d.defs import TFDAData, TFDADefault3DParams, DTFT, TFbF, TFbT, nan, pi
from official.projects.volumetric_models.data_augmentations_tf.tfda_2d.data_processing_utils import get_batch_size, update_tf_channel
...
class Parser(parser.Parser):
  ...
  def _data_augmentation_tr(self, image, label):
    image, label = process_batch2d(image, label[tf.newaxis,], [65, 65], [56, 40], 1)
    image, label = tf_tr_transforms(image, label)
    return image[0], label[0]

  def _data_augmentation_val(self, image, label):
    image, label = process_batch2d(image, label[tf.newaxis,], [56, 40], [56, 40], 1)
...
def tf_tr_transforms(images, segs):
  ...
  data_dict = GaussianBlurTransform2D(
    (0.5, 1.), 
